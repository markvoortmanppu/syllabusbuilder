...
function createAndUploadPdf(credentials, templatedata, syllabidata, sectionid, cb) {
  ...
  fs.writeFile(tmpname.replace(".pdf", ".md"), template.replace("![](https://mvoortman.it.pointpark.edu/logo.png)", "![](https://mvoortman.it.pointpark.edu/logo.png){ width=150px }"), function(err) {
    exec("pandoc -f markdown+hard_line_breaks -V colorlinks=true -V linkcolor=blue -V urlcolor=blue -V toccolor=blue --self-contained --pdf-engine=xelatex " + tmpname.replace(".pdf", ".md") + " -o " + tmpname, (error, stdout, stderr) => {
      if (error) {
