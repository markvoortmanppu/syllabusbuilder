...
function saveValuesToCookie(token, res) {
  ...
  res.cookie("graph_user_email", email, {maxAge: 3600000, httpOnly: true});
  // Save the user's admin status in a cookie
  res.cookie("graph_user_admin", process.env.ADMINS.split(" ").indexOf(email) >= 0 ? "true" : "", {maxAge: 3600000, httpOnly: true});
...
function clearCookies(res) {
  ...
  res.clearCookie("graph_user_admin", {maxAge: 3600000, httpOnly: true});
